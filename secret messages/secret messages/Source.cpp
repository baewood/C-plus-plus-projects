// Secret Messages witha substitution cipher
...
* This method reads the substitution chars *
* in from a file and stores them in the    *
* codeChar array. It also sets member	   *
* variable size.						   *
*******************************************/
...
CodeMaker::CodeMaker()
{
	size = 94;
	ifstream inputFile;
	inputFile.open("code.dat"); // Open the file, make sure it's in the same area as source.cpp

	for (int ascii = 32; ascii < 127; ascii++) // Reads data
...
/******************************************
*		CodeMaker::encode				  *
*This method encodes and returns a clear  *
* text stream.							  *
******************************************/
...
string CodeMaker::encode(string s)
{
	int ascii;
	char newChar;
	string newString = "";  // Will hold encoded string
...
string CodeMaker::decode(string s)
{
	...
	for (unsigned pos = 0; pos < s.length(); pos++)
	{
		...
		//Call findIt to find it int he array and return its index
		index = findIt(codeChar, size, nextChar);

		// Get the original character by computing its ASCII code
		originalChar = index + 32;

		// Concatenate this character onto the end of the decoded text string being constructed
...
/**********************************************
*				CodeMaker::findIt			  *
* This method performs a linear search on a   *
* character array looking for value.		  *
**********************************************/
...
/***************************************************
*				main							   *
* The client "program" that uses the CodeMaker     *
*class.											   *
***************************************************/
...
int main()
{
	...
	// Send the encoded text back to be decoded and display the result
