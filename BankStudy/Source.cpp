...
int main()
{
	...
	int pin2[NUM_DIGITS] = { 2, 4, 6, 8 }; //One element is different from PIN1

	int pin3[NUM_DIGITS] = { 1, 2, 3, 4 }; //All elements are different from PIN!
	...
	if (testPIN(pin1, pin1, NUM_DIGITS))
		...
		cout << "ERROR: pin1 and pin1 are erroreously identified "
			<< "as different.\n";
...
/****************************************************************************
*								testPIN										*
* This Boolyean function accepts and compares the values stored in two int  *
* arrays. If they both have exactly the same set of values, true is returned*
* If there are any differences, fase is returned.							*
****************************************************************************/
...
bool testPIN(const int custPIN[], const int databasePIN[], int size)
{
	for (int index = 0; index < size; index++)
	{
		if (custPIN[index] != databasePIN[index])
			return false;  // two different values
	}
